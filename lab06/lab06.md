...
3. Значение в поле Length (Длина) – это длина чего?
   - 88 байт: Сумма байт заголовка (8 байт) и сумма данных в пакете (80 байт) 
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - 2^16 - количество байт на заголовок (8 байт). 65535-8= 65527 байт
...
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   ...
   - Взаимосвязь между номерами портов заключается в том, что порт источника отправленного сообщения является портом назначения полученного сообщения. Порт назначения для отправленного сообщения также является портом источника для полученного сообщения.
